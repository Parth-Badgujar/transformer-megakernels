...
class LLMMegaKernel:
    ...
    def _get_smem_storage(self):
        ...
        STAGE_ELEMS = max(bM * bK + bN * bK, bR * E)
        STAGES      = nS
        ...
        class BarrierStorage:
            load_barrier:    cute.struct.MemRange[Uint64, STAGES]
        ...
        class SharedStorage:
            ...
            sW:     cute.struct.Align[cute.struct.MemRange[BFloat16, SW_ELEMS],             128]
            stages: cute.struct.Align[cute.struct.MemRange[BFloat16, STAGES * STAGE_ELEMS], 1024]
            out:    cute.struct.Align[cute.struct.MemRange[BFloat16, OUT_ELEMS],            1024]
    ...
    def __call__(
        ...
        mWS1_ME = cute.make_tensor(
            mWorkspace1.iterator, cute.make_layout((M_tokens, embed_dim),  stride=(embed_dim,  1)))
        mWS2_MF = cute.make_tensor(
            mWorkspace2.iterator, cute.make_layout((M_tokens, ff_dim),  stride=(ff_dim,  1)))
        ...
        sA_atom = cute.make_layout(
            shape = (bM, bK),
            stride = (bK, 1)
        )

        PAD = self.output_pad
        sC_atom = cute.make_layout(
            shape = (bM, 1, bN + PAD),
            stride = (bN + PAD, bN + PAD, 1)
        )
        sW_atom = cute.make_layout(
            shape = (1, bN, bK),
            stride = (bN * bK, bK, 1)
        )
        ...
        sE_row  = cute.make_layout(
            shape = (1, embed_dim),
            stride = (embed_dim, 1)
        )
        sE_rows = cute.make_layout(
            shape = (self.bR, embed_dim),
            stride = (embed_dim, 1)
        )
        ...
        tma_rms_w,  g_rms_w  = cpasync.make_tiled_tma_atom(load_op, mRMS_weights, sE_row, (1, embed_dim))
        ...
        mWS2_MQ_st = cute.make_tensor(
            mWorkspace2.iterator, cute.make_layout((M_tokens, Qd // bN, bN), stride=(Qd, bN, 1))
        )
        mWS2_MF_st = cute.make_tensor(
            mWorkspace2.iterator, cute.make_layout((M_tokens, ff_dim  // bN, bN), stride=(ff_dim,  bN, 1))
        )
        mEmb_st    = cute.make_tensor(
            mEmbedding.iterator, cute.make_layout((M_tokens, embed_dim // bN, bN), stride=(embed_dim, bN, 1))
        )

        tma_ws1_A,    g_ws1_A    = cpasync.make_tiled_tma_atom(load_op,  mWS1_ME, sA_swz,  (bM, bK))
        tma_ws1_rows, g_ws1_rows = cpasync.make_tiled_tma_atom(store_op, mWS1_ME, sE_rows, (self.bR, embed_dim))
        ...
        tma_emb_rows, g_emb_rows = cpasync.make_tiled_tma_atom(load_op,  mEmbedding, sE_rows, (self.bR, embed_dim))
        tma_emb_red,  g_emb_red  = cpasync.make_tiled_tma_atom(emb_store_op, mEmb_st, sC_atom, (bM, 1, bN + PAD))

        sO_tile = cute.make_layout(
            shape = (1, 1, self.bQ, d + PAD),
    ...
    def kernel(self,
        ...
        if warp_id == 0:
            with cute.arch.elect_one():
                load_stage[0]      = Int32(0)
                load_phase_cell[0] = Int32(0)
        ...
        compute_mbar_phase = Int32(0)
        input_mbar_phase   = Int32(0)
        output_mbar_phase  = Int32(0)
