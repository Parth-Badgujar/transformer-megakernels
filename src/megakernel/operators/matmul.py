...
class Matmul:
    ...
    def run(
        ...
        load_A = partial(
            self._load_A,
            tma_A = tma_A, tAgA = tAgA, tAsA = tAsA, load_bar = load_bar,
            warp_id = warpgroup.warp_id
        )
        load_B = partial(
            self._load_B,
            tma_B = tma_B, tBgB = tBgB, tBsB = tBsB,
            load_bar = load_bar,
            warp_id = warpgroup.warp_id
        )
        expect_tx = partial(
            self._expect_tx, load_bar = load_bar, warp_id = warpgroup.warp_id
        )
        warpgroup_sync = partial(
            self._warpgroup_sync, group_id = warpgroup.group_id
        )
        wait_prev = partial(
            self._wait_prev,
        ...
        if warpgroup.warp_id == 0:
            ...
            sC_part, gC_part = cpasync.tma_partition(
                tma_C, 0, cute.make_layout(1), sC_g, gC_tma_g)
