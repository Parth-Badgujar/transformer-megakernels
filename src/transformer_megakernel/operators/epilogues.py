...
def silu_mul(*, tiled_mma, tCrC, sC, warpgroup, gC, gC_tma=None, pid_m, pid_n, bM, bN, **_):
    ...
    store_atom = cute.make_copy_atom(cute.nvgpu.warp.StMatrix8x8x16bOp(num_matrices=4),
                                     cutlass.BFloat16)
    thr_cpy    = cute.make_tiled_copy_C(store_atom, tiled_mma).get_slice(tid)

    rUp = cute.make_fragment_like(tCrC, cutlass.BFloat16)   # full bf16; holds `up`, then the result
    cute.autovec_copy(tCgC, rUp)                            # up (bf16) -> rUp

    n_tiles = tCrC.shape[2]                                 # MMA_N register-tiles (static int)
